...
// TestCondense_reportsReasoningToTheFallbackCounter pins the seam between two
// changes that were developed apart: the counter the eval harness reads, and
// the reasoning-aware error. A degraded run has to be countable *and* say why,
// or the harness reports a number it cannot explain.
func TestCondense_reportsReasoningToTheFallbackCounter(t *testing.T) {
	var reasons []string
	c := rewrite.Condense{
		Chat:       chatReasoning("Okay, the user wants a rewrite. Let me consider"),
		OnFallback: func(reason string) { reasons = append(reasons, reason) },
	}
	if _, err := c.Queries(context.Background(), nil, "and maps?"); err != nil {
		t.Fatalf("Queries: %v", err)
	...
	if !strings.Contains(reasons[0], "reasoning") {
		t.Errorf("the counter was given %q, which does not say the model reasoned", reasons[0])
	}
}
