...
| `tbuk ask` warns `could not condense the question (…)` | The `condense` rewrite failed, timed out, or came back empty or absurd | Nothing is broken — the query was planned with the `window` instead, and the answer is grounded as usual. The call is capped at 20s and 256 tokens, so a large model is the usual cause, and a reasoning model is the usual cause of "returned nothing": its thinking fills the budget before the question appears. Serve a small model for rewriting (a 4B is ample — the task is one line), check the template's `model:`, or set `retrieval.rewrite: window` |
