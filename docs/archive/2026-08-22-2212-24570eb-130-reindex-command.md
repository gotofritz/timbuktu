...
> **Superseded in part — read this first.** `tbuk reindex` shipped as designed.
> The import/restore half did not: it was redesigned during review and the
> shape below is not what exists.
>
> - **`tbuk restore` does not exist.** Decision 9 kept today's verbatim restore
>   under a new name. It was dropped instead: its correctness depends on source
>   and target embedding models being identical, which tbuk cannot verify —
>   matching dimensions do not prove matching models, and the failure is silent
>   nonsense rather than an error. Rewinding one's own knowledge base is
>   `tar -xf backup.tar -C ~/.tbuk`, no command needed.
> - **`tbuk import` does not restore the archive.** Decisions 10–13 built import
>   as "verbatim restore, then re-encode". It now takes only the archived source
>   files under `raw/` plus the database read as a *manifest* (path, title, mime,
>   SHA256, user metadata) and embeds them locally. The archive's config,
>   extracted cache and prompts are never written; its embeddings are never read.
> - **Decision 11's scoping problem was a false premise.** It assumed row-level
>   merging that `internal/importer` never did. Import now creates or updates
>   rows itself, keyed on `documents.path`.
> - **`--merge` / `--force-config` / `--force-data` are gone**, replaced by
>   `--on-conflict skip|overwrite|ask` (default `skip`, so a repeat import is a
>   no-op), plus `--dry-run` and `--yes`.
> - **`--topic` did not ship** (decision 6): topics do not exist yet.
>   `docs/plans/32-topics.md` owns wiring it in.
